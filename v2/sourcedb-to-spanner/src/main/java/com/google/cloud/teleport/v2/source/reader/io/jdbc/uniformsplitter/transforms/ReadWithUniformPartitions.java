...
@AutoValue
public abstract class ReadWithUniformPartitions<@UnknownKeyFor @NonNull @Initialized T>
        ...
        ranges.apply(
            ParDo.of(
                MergeRangesDoFn.builder()
                    ...
                    .build()));
    mergedRanges = peekRanges(mergedRanges);
    return mergedRanges
  ...
  public Builder<T> builder() {
    return new AutoValue_ReadWithUniformPartitions.Builder<>();
  }
  ...
  private PCollection<ImmutableList<Range>> addStage(
    ...
    PCollection<ClassifiedRanges> classifiedRanges = input.apply(ParDo.of(classifierFn));

    PCollection<Range> countedRanges =
        classifiedRanges
            .apply(
                MapElements.via(
                    new SimpleFunction<ClassifiedRanges, ImmutableList<Range>>() {
                      @Override
                      public ImmutableList<Range> apply(ClassifiedRanges input) {
                        return input.toCount();
                      }
                    }))
            .apply(Flatten.iterables())
            .apply(rangeCountTransform);

    PCollection<Range> rangesWithNewColumns =
        classifiedRanges
            .apply(
                MapElements.via(
                    new SimpleFunction<ClassifiedRanges, ImmutableList<ColumnForBoundaryQuery>>() {
                      @Override
                      public ImmutableList<ColumnForBoundaryQuery> apply(ClassifiedRanges input) {
                        return input.toAddColumn();
                      }
                    }))
            .apply(Flatten.iterables())
            .apply(rangeBoundaryTransform)
            .apply(ParDo.of(new SplitRangeDoFn()));

    PCollection<Range> retainedRanges =
        classifiedRanges
            .apply(
                MapElements.via(
                    new SimpleFunction<ClassifiedRanges, ImmutableList<Range>>() {
  ...
  private PCollection<ImmutableList<Range>> peekRanges(
    ...
    if (rangesPeek() != null) {
      return mergedRanges
          .apply(Wait.on((PCollection<?>) mergedRanges.apply(rangesPeek())))
          .apply(
              MapElements.via(
                  new SimpleFunction<ImmutableList<Range>, ImmutableList<Range>>() {
                    @Override
                    public ImmutableList<Range> apply(ImmutableList<Range> input) {
                      return input;
                    }
                  }));
    } else {
  ...
  public abstract static class Builder<T> {
    ...
    public ReadWithUniformPartitions<T> build() {
      ...
      if (!splitStageCountHint().isPresent()) {
        Long splitHeightHint = logToBaseTwo(maxPartitionsHint) + partitionColumns().size();
