...
public final class MysqlDialectAdapter implements DialectAdapter {
   ...
   */
  @Override
  public String getBoundaryQuery(String tableName, ImmutableList<String> partitionColumns) {
    // Using String builder as the string is built in a loop.
    StringBuilder caseBuffer = new StringBuilder("CASE ");
    for (String col : partitionColumns) {
      caseBuffer.append(" WHEN (?) THEN " + col);
    }
    caseBuffer.append(" END");
    String caseColSelect = caseBuffer.toString();
    // Implementation details, for some databases Min and Max are not allowed to take dynamic
    // paramters.
    return addWhereClause(
        String.format("select MIN(%s),MAX(%s) from %s", caseColSelect, caseColSelect, tableName),
        partitionColumns);
  }
