...
public class MysqlDialectAdapterTest {
  ...
  public void testGetBoundaryQuery() {
    String testTable = "testTable";
    ImmutableList<String> cols = ImmutableList.of("col_1", "col_2");
    assertThat(new MysqlDialectAdapter(MySqlVersion.DEFAULT).getBoundaryQuery(testTable, cols))
        .isEqualTo(
            "select MIN(CASE  WHEN (?) THEN col_1 WHEN (?) THEN col_2 END),MAX(CASE  WHEN (?) THEN col_1 WHEN (?) THEN col_2 END) from testTable where ((? = FALSE) OR (col_1 >= ? AND (col_1 < ? OR (? = TRUE AND col_1 = ?)))) AND ((? = FALSE) OR (col_2 >= ? AND (col_2 < ? OR (? = TRUE AND col_2 = ?))))");
  }
